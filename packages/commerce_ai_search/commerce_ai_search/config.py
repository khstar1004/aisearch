...
def _apply_aisearch_env_aliases() -> None:
    """Accept productized AISEARCH_* env names while preserving HAEORUM_* compatibility."""
    for name, value in list(os.environ.items()):
        if not name.startswith("AISEARCH_"):
            continue
        legacy_name = "HAEORUM_" + name[len("AISEARCH_") :]
        if legacy_name not in os.environ:
            os.environ[legacy_name] = value
    if "AISEARCH_MARQO_URL" in os.environ and "MARQO_URL" not in os.environ:
        os.environ["MARQO_URL"] = os.environ["AISEARCH_MARQO_URL"]
