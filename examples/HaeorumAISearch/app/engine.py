...
@dataclass
class QueryImageEmbeddingBatchItem:
    image: str
    event: threading.Event
    vector: list[float] | None = None
    exception: BaseException | None = None
...
class QueryImageEmbeddingBatcher:
    def __init__(self, max_batch_size: int, wait_seconds: float):
        self.max_batch_size = max(1, int(max_batch_size))
        self.wait_seconds = max(0.0, float(wait_seconds))
        self._lock = threading.RLock()
        self._pending: list[QueryImageEmbeddingBatchItem] = []
        self._submitted = 0
        self._batch_count = 0
        self._batched_input_count = 0
        self._max_observed_batch_size = 0

    @property
    def enabled(self) -> bool:
        return self.max_batch_size > 1 and self.wait_seconds > 0

    def embed(self, image: str, embed_batch: Any, wait_timeout_seconds: float) -> list[float]:
        if not self.enabled:
            return list(embed_batch([image])[0])

        item = QueryImageEmbeddingBatchItem(str(image), threading.Event())
        leader = False
        flush_batch: list[QueryImageEmbeddingBatchItem] | None = None
        with self._lock:
            self._submitted += 1
            self._pending.append(item)
            if len(self._pending) >= self.max_batch_size:
                flush_batch = self._take_pending_locked()
            elif len(self._pending) == 1:
                leader = True

        if flush_batch is not None:
            self._flush(flush_batch, embed_batch)
        elif leader:
        ...
        timeout_seconds = max(float(wait_timeout_seconds or 0.0), self.wait_seconds + 0.001)
        if not item.event.wait(timeout=timeout_seconds):
            raise TimeoutError("query image embedding batch did not complete before timeout")
        if item.exception is not None:
            raise item.exception
        if item.vector is None:
            raise RuntimeError("query image embedding batch returned no vector")
        return list(item.vector)

    def _take_pending_locked(self) -> list[QueryImageEmbeddingBatchItem]:
        batch = self._pending[: self.max_batch_size]
        del self._pending[: len(batch)]
        return batch

    def _flush(self, batch: list[QueryImageEmbeddingBatchItem], embed_batch: Any) -> None:
        if not batch:
            return
        try:
            vectors = embed_batch([item.image for item in batch])
            if len(vectors) != len(batch):
                raise RuntimeError(
                    f"query image embedding batch returned {len(vectors)} vectors for {len(batch)} inputs"
                )
            ...
            with self._lock:
                self._batch_count += 1
                self._batched_input_count += len(batch)
    ...
    def status(self) -> dict[str, Any]:
        with self._lock:
            ...
            batched_input_count = self._batched_input_count
        ...
        return {
            ...
            "batched_input_count": batched_input_count,
            "avg_batch_size": round(batched_input_count / batch_count, 3) if batch_count else 0.0,
...
class MarqoSearchEngine(SearchEngine):
    ...
    def __init__(
        ...
        self.qwen_query_image_batch_size = max(1, int(qwen_query_image_batch_size))
        self.qwen_query_image_batch_wait_ms = max(0.0, float(qwen_query_image_batch_wait_ms))
        self._qwen_query_image_batcher = QueryImageEmbeddingBatcher(
            self.qwen_query_image_batch_size,
            self.qwen_query_image_batch_wait_ms / 1000.0,
        )
    ...
    def apply_text_auxiliary_scores(
        ...
        *,
        ...
        prepared_filters: PreparedProductFilters | None = None,
    ) -> list[EngineHit]:
        auxiliary_hits = auxiliary_hits_by_document_key(auxiliary_data)
        ...
        next_rank = len(weighted)
        auxiliary_only_multiplier = max(0.25, 1.0 - weight)
        for document_key, hit in auxiliary_hits.items():
    ...
    def health(self) -> dict[str, Any]:
        ...
        if provider_is_gemini:
            ...
            request_policy["gemini_query_image_batch_size"] = self.qwen_query_image_batch_size
            request_policy["gemini_query_image_batch_wait_ms"] = self.qwen_query_image_batch_wait_ms
            ...
            request_policy["qwen_query_image_batch_size"] = self.qwen_query_image_batch_size
            request_policy["qwen_query_image_batch_wait_ms"] = self.qwen_query_image_batch_wait_ms
    ...
    def qwen_image_query_vector(self, query: EngineQuery) -> tuple[list[float], str, str]:
        ...
        vector = self._compute_qwen_query_vector_once(
            cache_key,
            lambda: self._qwen_query_image_batcher.embed(
                query.image_data_url or "",
                lambda images: self.qwen_embed_images(
                    images,
    ...
    def qwen_query_vector_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "image_batcher": self._qwen_query_image_batcher.status(),
        }
...
def auxiliary_hits_by_document_key(data: Mapping[str, Any]) -> dict[str, EngineHit]:
    hits: dict[str, EngineHit] = {}
    for raw_hit in data.get("hits", []) if isinstance(data, Mapping) else []:
        if not isinstance(raw_hit, dict):
            continue
        try:
        ...
        if not math.isfinite(score):
            continue
        document_key = product_document_id(product.mall_id, product.product_id)
        previous = hits.get(document_key)
        if previous is None or score > previous.score:
            hits[document_key] = EngineHit(
                document=product,
                score=score,
                source_scores={"marqo": score},
            )
