...
def check_compose_defaults() -> dict[str, Any]:
    ...
    if "replace-with-google-cloud-project-id" not in gemini_text and "GEMINI_QUOTA_PROJECT" in gemini_text:
    ...
    if 'GEMINI_AUTH_MODE: "${GEMINI_AUTH_MODE:-adc}"' not in localtest_text:
        gemini_override_problems.append("local ADC test compose must set adc auth explicitly")
    if "GEMINI_QUOTA_PROJECT: \"${GEMINI_QUOTA_PROJECT:?" not in localtest_text:
        gemini_override_problems.append("local ADC test compose must require GEMINI_QUOTA_PROJECT explicitly")
    return {
...
def check_demo(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if "Gemini" not in text:
        problems.append("demo does not display Gemini")
