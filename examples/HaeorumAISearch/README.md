...
현재 반입 기준 기본 구성은 **Marqo + Gemini embedding API**입니다. 기본 API stack 컨테이너는 `ai-search`, `gemini-embedding`, `marqo-api`, `mioc`, `vespa` 다섯 개이고, 운영에서는 MSSQL 증분 동기화용 `sync-worker`를 추가해 상시 여섯 개를 띄웁니다. 로컬 GPU 임베딩 컨테이너는 이 구성에서 띄우지 않습니다. 실행 상태 기준은 `deploy/runtime-stack-gemini-marqo.md`에 고정했습니다.
...
ADC/OAuth로 quota project를 분리해야 하는 배포에서는 아래처럼 명시적으로 ADC를 선택할 수 있습니다. 기본 운영과 로컬 비용 테스트는 API key 방식입니다.
...
Docker로 Marqo stack과 Gemini 프록시를 함께 띄우려면 override compose를 함께 사용합니다. 운영과 로컬 비용 테스트 모두 API key 방식이며, `compose-haeorum-gemini.yaml`만 추가하면 됩니다.
...
동일한 JCLGift 로컬 테스트 값을 덮어쓰려면 아래처럼 별도 override를 추가할 수 있습니다. 이 override도 API key를 요구합니다.
...
$env:HAEORUM_MSSQL_QUERY="SELECT CAST(p_idx AS varchar(100)) AS product_id, product_name, LEFT(COALESCE(NULLIF(category_name1, ''), NULLIF(category_name2, ''), NULLIF(category_name3, ''), NULLIF(category_name4, ''), ''), 100) AS category_name, TRY_CONVERT(float, price) AS price, TRY_CONVERT(float, price) AS price_min, TRY_CONVERT(float, price) AS price_max, CAST('' AS nvarchar(4000)) AS print_methods, CAST('' AS nvarchar(4000)) AS materials, CAST('' AS nvarchar(4000)) AS colors, CAST(NULL AS int) AS min_order_qty, CAST(NULL AS int) AS delivery_days, CAST(p_idx AS varchar(100)) AS product_group_id, main_image_url, product_url, status, updated_at, CAST(CASE WHEN status = '승인' THEN 0 ELSE 1 END AS bit) AS is_deleted, CASE WHEN status = '승인' THEN 'Y' ELSE 'N' END AS display_yn, CAST(mall_id AS varchar(64)) AS mall_id FROM dbo.v_ai_search_products WHERE p_idx IS NOT NULL AND product_name IS NOT NULL AND LTRIM(RTRIM(product_name)) <> ''"
$env:HAEORUM_MSSQL_PRODUCT_ID_COLUMN="product_id"
