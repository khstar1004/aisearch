...
| Missing or deleted Vespa persistent volume | Recreating `vespa` or `marqo-api` makes `/indexes` empty and all search results disappear | `compose-haeorum-marqo.yaml` mounts `vespa-data` at `/opt/vespa/var`; start `sync-worker` with `--no-deps`; never run `docker compose down -v` for routine rollout | `docker volume ls`, `docker inspect` Vespa mounts, `/indexes`, post-restart API smoke |
| Vespa disk guardrail exception on server82 | Reindex/feed stops with backend disk-space errors even when the host still has free bytes, or the node runs too close to full-disk operation | Server82 currently keeps Docker data under `/home/docker`; `VESPA_DISK_USAGE_LIMIT=0.95` is a temporary go-live exception because `/home` is already high usage. Clean or move non-AI data after launch and lower the limit back toward the default guardrail. | `df -h /home`, `/health` backend `storageUsedPercentage`, Marqo/Vespa feed logs, post-reindex Docker stats |
...
| Temporary MSSQL certificate trust exception | AI search accepts a self-signed SQL Server certificate; a network-level attacker could impersonate the DB endpoint, expose read-only credentials, or feed fake product data into the index | Server82 go-live permits `Encrypt=yes;TrustServerCertificate=yes` only with `HAEORUM_MSSQL_ALLOW_TRUST_SERVER_CERTIFICATE=true`; DB access is IP-restricted to `222.236.45.35`, account is read-only, and SQL Server certificate remediation remains a post go-live hardening item | `deploy/server-db-intake.md`, `deploy/tomorrow-go-live-checklist.ko.md`, `env_check.py`, MSSQL View check logs |
